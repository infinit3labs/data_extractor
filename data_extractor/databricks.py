...
class DatabricksDataExtractor:
    ...
    def _setup_databricks_logging(self):
        ...
        if self.use_existing_spark:
            ...
            if not hasattr(self._local, 'spark'):
                self._local.spark = SparkSession.builder \
                    .appName(f"DataExtractor-Databricks-{threading.current_thread().name}") \
    ...
    def extract_table(self,
        ...
        try:
            ...
            self.logger.info(f"[{thread_name}] Starting Databricks extraction for table: {table_name}")
            ...
            df = spark.read \
                ...
                .option("numPartitions", str(min(8, spark.sparkContext.defaultParallelism))) \
                .load()
                
            # Check if data was extracted
            record_count = df.count()
            self.logger.info(f"[{thread_name}] Extracted {record_count} records from {table_name}")
            
            if record_count == 0:
                self.logger.warning(f"[{thread_name}] No data found for table {table_name}")
            ...
            self.logger.info(f"[{thread_name}] Successfully extracted table: {table_name}")
            return True
            
        except Exception as e:
            self.logger.error(f"[{thread_name}] Error extracting table {table_name}: {str(e)}")
            return False
    ...
    def extract_tables_parallel(self, table_configs: List[Dict]) -> Dict[str, bool]:
        ...
        self.logger.info(f"Starting Databricks parallel extraction of {len(table_configs)} tables using {self.max_workers} workers")
        ...
        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            ...
            for future in as_completed(future_to_table):
                ...
                try:
                    ...
                    if success:
                        self.logger.info(f"Successfully completed Databricks extraction for table: {table_name}")
                    else:
                        self.logger.error(f"Failed Databricks extraction for table: {table_name}")
                        
                except Exception as e:
                    self.logger.error(f"Exception during Databricks extraction of table {table_name}: {str(e)}")
                    results[table_name] = False
                    
        # Log summary
        successful = sum(1 for success in results.values() if success)
        total = len(results)
        self.logger.info(f"Databricks parallel extraction completed: {successful}/{total} tables successful")
