...
class DataExtractor:
    ...
    def extract_table(self,
        ...
        try:
            ...
            self.logger.info(f"[{thread_name}] Starting extraction for table: {table_name}")
    ...
    def extract_tables_parallel(self, table_configs: List[Dict]) -> Dict[str, bool]:
        ...
        """
        self.logger.info(f"Starting parallel extraction of {len(table_configs)} tables using {self.max_workers} workers")
        ...
        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            ...
            for future in as_completed(future_to_table):
                ...
                try:
                    ...
                    if success:
                        self.logger.info(f"Successfully completed extraction for table: {table_name}")
                    else:
                        self.logger.error(f"Failed extraction for table: {table_name}")
                        
                except Exception as e:
                    self.logger.error(f"Exception during extraction of table {table_name}: {str(e)}")
                    results[table_name] = False
                    
        # Log summary
        successful = sum(1 for success in results.values() if success)
        total = len(results)
        self.logger.info(f"Parallel extraction completed: {successful}/{total} tables successful")
