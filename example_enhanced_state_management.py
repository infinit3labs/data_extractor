...
def main():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        print(
            f"✅ Initial run completed: {sum(results.values())}/{len(results)} tables successful"
        )
        ...
        print(
            f"✅ Second run completed: {sum(results2.values())}/{len(results2)} tables processed"
        )
        ...
        if success:
            ...
            results3 = simulate_extraction_with_state_management(
                extractor, table_configs[:1]
            )
        ...
        if not validation["is_consistent"]:
            ...
            for inconsistency in validation["inconsistent_extractions"]:
                print(
                    f"   - {inconsistency['table_key']}: {inconsistency['extraction_date']}"
                )
        ...
        print(
            f"   Total Tables: {report.get('extraction_summary', {}).get('total_extractions', 0)}"
        )
        ...
        if status_resumed:
            print(
                f"✅ Resumed pipeline - Restart count: {status_resumed.get('restart_count', 0)}"
            )
        else:
        ...
        for run in recent_runs:
            print(
                f"   - {run['run_id']}: {run['status']} ({run['completion_rate']:.1f}%)"
            )
...
def simulate_extraction_with_state_management(extractor, table_configs):
    ...
    for config in table_configs:
        ...
        table_key = (
            f"{config['source_name']}.{config.get('schema_name', '')}.{table_name}"
        )
