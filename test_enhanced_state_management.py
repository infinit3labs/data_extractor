...
def test_enhanced_state_management():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        if state_manager.start_extraction(orders_key, "thread-2"):
            ...
            # Simulate failure
            state_manager.complete_extraction(
                table_key=orders_key, success=False, error_message="Connection timeout"
            )
        ...
        for pipeline in recent:
            print(
                f"   - {pipeline['run_id']}: {pipeline['status']} ({pipeline['completion_rate']:.1f}%)"
            )
...
def demonstrate_24hour_window():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        if start_time and end_time:
            print(
                f"📅 Window Duration: {(end_time - start_time).total_seconds() / 3600} hours"
            )
        ...
        pipeline_id = state_manager.start_pipeline(
            table_configs=table_configs, extraction_date=yesterday
        )
